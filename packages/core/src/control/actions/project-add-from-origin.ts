...
export interface ProjectAddInput {
  ...
  remote: string;
  /** Destination parent dir on disk */
  parentDir: string;
  /** Optional branch to clone */
  branch?: string;
  /** Override target folder name (defaults to repo name from URL) */
  name?: string;
}
...
const validateProjectAddInput = (raw: unknown): ProjectAddInput => {
...
export const createProjectAddFromOriginAction = (deps: {
  ...
  action({
    ...
    validateInput: validateProjectAddInput,
