 ...
 *
 * Output: Writes changelog to stdout or to a file if --output is specified
 */

import Anthropic from '@anthropic-ai/sdk';
import { execSync } from 'child_process';

const CHANGELOG_PROMPT = `You are a changelog generator for Agentage AgentKit.
Agentage AgentKit is the agent SDK — @agentage/core provides the agent API and @agentage/platform provides daemon/hub protocol types.
...
### New Features
- Add workspace details page with tabbed navigation
- Add support for custom agent configurations

### Bug Fixes
- Fix memory leak in agent connection handling
...
function getCommitsSinceLastTag() {
  try {
    ...
    // Get commit messages with author and date
...
async function main() {
  ...
  if (!version) {
    ...
    console.error('Example: node generate-changelog.js core@0.5.1+platform@0.2.2');
    process.exit(1);
  }
  ...
  try {
    ...
    // Output the changelog to stdout (can be captured by other scripts)
