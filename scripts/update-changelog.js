...
`;
  ...
  // Escape special regex characters in version string (handles @, +, etc.)
  const escapedVersion = version.replace(/[.*+?^${}()|[\]\\@]/g, '\\$&');

  // Check if there's already an entry for this version
  const versionPattern = new RegExp(`^## \\[${escapedVersion}\\]`, 'm');
...
async function main() {
  ...
  if (!version) {
    ...
    console.error(
      '  echo "### New Features\\n- Add feature X" | node update-changelog.js core@0.5.1+platform@0.2.2'
    );
    process.exit(1);
  }
